...
function startsWithVariations(target: string, lookFor: string): boolean {
	/**
	 * verifica se uma string começa com uma sequência de caracteres,
	 * mas também inclue as variações com números que o alvo pode ter, que fazem o startsWith não identificar os caracteres
	 * como: main, 00_main, 000_main etc. essa func corta a string a partir de um ponto em que o lookFor ainda pode existir
	 */

	if (
		target.startsWith(lookFor) ||
		target.slice(3).startsWith(lookFor) ||
		target.slice(4).startsWith(lookFor)
	) {
		return true;
	} else {
		return false;
...
export class GlyphureOLD extends Plugin {
	...
	verifyDirectoryIcons(directories: NodeListOf<Element>) {
		directories.forEach(d => {
			...
			// definir se a pasta em questão tem um nome reservado pra ícones especiais
			// isso é tido como true até o switch dizer o contrário
			...
			const iconsDirectoryGlyph = 'traffic-cone';
			//const iconsDirectoryGlyph = 'flask-conical';
	...
	verifyFileIcons(files: NodeListOf<Element>) {
		files.forEach(f => {
			...
			// aquela etiqueta que aparece na direita do nome do arquivo e indica o formato dele se ele não for um .md
			// tipo "canvas", "base", "png" etc.
			...
			// return pra não adicionar ícones, só as tags
			return;
	...
	async injectIcons() {
		...
		for (const d of directories) {
			this.settings.rules.forEach(rule => {
				...
				// garantir que o id seja válido pra api do obsidian/lucide
				if (!rule.icon.startsWith('lucide-')) {
					rule.icon = 'lucide-' + rule.icon;
				}
